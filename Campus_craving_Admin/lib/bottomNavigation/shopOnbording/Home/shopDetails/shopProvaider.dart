...
class ShopProvaider with ChangeNotifier {
  ...
  void fatchShopDeatils() async {
    ...
    for (var e in Items.docs) {
      shopModel = ShopModel(
          ...
          imageUrl: e.get('ImageUrl'),
          shopName: e.get('ShopName'),
          adminPremision: e.get('AdminPermission'));
      newList.add(shopModel);
    }
    shopList = newList;
    notifyListeners();
  ...
  Future<void> updateAdminPremission(
      String? shopId, bool adminPermission) async {
    try {
      await shopCollection.doc(shopId).update({
        'AdminPermission': adminPermission,
      });
      print('Admin permission updated successfully');
    } catch (e) {
      print('Error updating admin permission: $e');
    }
  }
